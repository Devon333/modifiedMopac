...
! Set up Reimers data
        if (.not. allocated(x))     allocate(x(numat))
        if (.not. allocated(y))     allocate(y(numat))
        if (.not. allocated(z))     allocate(z(numat))
        if (.not. allocated(xz))    allocate(xz(numat,3))
        if (.not. allocated(zcore)) allocate(zcore(numat))
        if (.not. allocated(beta))  allocate(beta(mpack))
        if (.not. allocated(gamma)) allocate(gamma(norbs,norbs))
        gamma = 0.d0
        if (.not. allocated(s))     allocate(s(mpack))
        if (.not. allocated(betao)) allocate(betao(norbs))
        if (.not. allocated(ibf))   allocate(ibf(numat))
        if (.not. allocated(natm))  allocate(natm(numat))
        if (.not. allocated(r))     allocate(r(numat,numat))
        if (.not. allocated(nbf))   allocate(nbf(numat))
        if (.not. allocated(nbt))   allocate(nbt(norbs))
        if (.not. allocated(nprn))  allocate(nprn(norbs))
        if (.not. allocated(iat))   allocate(iat(norbs))
        if (.not. allocated(natt))  allocate(natt(norbs))
