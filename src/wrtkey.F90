...
subroutine wrtchk (allkey)
  ...
  integer :: i, j, k, l, nmos, indo
  character :: ch, ch1
  character (len=1000), intent (inout) :: allkey
  double precision, external :: reada
  indo  = (Index (keywrd, " INDO") /= 0)
  ...
  if (.not. (birad .or. exci .or. ci .or. indo .or. (index(keywrd, " OPEN") /= 0))) then
...
!
  ...
  if (myword(allkey,' MAXCI='))  write (iw, '(" *  MAXCI =",i5," maximum CI states computed")') nint(reada(keywrd,index(keywrd, ' MAXCI')))
  if (myword(allkey,' WRTCI='))  write (iw, '(" *  WRTCI =",i5," maximum CI states printed")') nint(reada(keywrd,index(keywrd, ' WRTCI')))
  if (myword(allkey,' TDIP'))    write (iw, '(" *  TDIP       -  Print transition dipoles between excited states")')
  if (myword(allkey,' WRTCONF='))write (iw, '(" *  WRTCONF =",f8.4," - Print configurations with coefficients greater than cutoff")') reada(keywrd,index(keywrd, ' WRTCONF'))
!
...
!
  ...
  if (myword(allkey, " GEO_REF")) then
    ...
    if (keywrd(j + 1:j + 1) == " ") then
        write(iw,'(a)') " *               (A DEFAULT BIAS OF 3.0"// &
...
10870 format (" *  OMIN=      - MINIMUM EIGENVECTOR OVERLAP IN TS", f7.3)
  ...
  if (myword(allkey, " GNORM")) then
    ...
    ch4 = char(ichar("6") + max(0, min(2,int(log10(sum_2)))))//".3"
