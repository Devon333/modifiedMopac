...
! ____________________________________________________________________________
      ...
      subroutine excite (tm,istsym,e,ci)
...
!     **** corrections to all of the CI matrix elements.		****
      ...
      use reimers_C, only:  n, nb2, matind, nbt, nel, nshell, norb, &
          ...
          icisym, e0, nmp2, nmp2t, ismp2, nochk, edef, ecore, eec, ee2, &
...
!     **** MP2 storage in excite and spcls ****
      parameter         mmp2= 10000
      double precision  tm(nb2,3), e(nconf), ci(matind(nconf)+nconf+1), &
                        ener, tel, xj12, xk12
      integer           vvs(4), vvd(4), a1, a, b, b1, maxunp, &
                        istsym(nconf), &
                        i1, icifrag1, iii, iirefd, irefd, ismin, ispn, &
                        isynca, isyncb, ix, &
                        j1, jconf, jconfm, &
                        jmin, kline, maxoc, &
                        nalpha, nbeta, ndif, nelopn, nend, norbopn, nov2, nov4, &
                        nperm, nperma, npermb, nv, nvs, nvd
      double precision, allocatable :: aii(:)
      logical           exok, maxmp2
      logical*1         permca,ll,default
      logical*1, allocatable :: aoc(:),boc(:)
      character*8       param
      integer           nsorb(10),i,j,k
      real (double)     ediel
 1230 format (//,' Reference determinate nber',i4,' is ',a5,' 1 -',25i4)
 1231 format (' Reference determinate nber',i4,' is ',a5,50l2)
 1300 format (' CAS excitations amongst orbitals:',25i4)
 1310 format (1x,a6,' excitations FROM orbs',i4,' to',i4,' INTO orbs',i4,&
     &       ' to',i4)
 1320 format (/' CI excitations=',i5,':',8(5x,a3,'=',i3))
 1340 format (/' MP2 index storage=',i8,' nber states=',i8)
...
!     **** if type j= s,px,py,etc is present, then nbtmo bit j is set to 1 ****
      ...
      do i= 1,n
        ...
        do k= 1,n
          if (abs(cc0(i,k)).gt.1.d-5) ifbinc(nbt(k))= 1
        end do
        nbtmo(i)= 0
        do j= 8,0,-1
...
!     **** apply symmetry weights to MO coeffs ****
      ...
      if(.not. allocated(isc )) allocate(isc(n))
      if(.not. allocated(istr)) allocate(istr(8,n))
...
!     ******************** form determinants ***************************
!     **** loop over reference determinates; first read alpha occupancy ****
...
!	**** alpha set occupied/vacant flags ****
        ...
        write (iw,1230) irefd,'ALPHA',(vv(i),i=1,nv)
...
!	**** spin adapt and include in config list ****
        ...
        call spclas (tm,ci,istsym,jconf, aor,bor,ao1,bo1, e,&
...
!         **** ensure CAS orbitals are in ascending order for sort speed ***
          ...
          if (vv(1).le.ncore .or. vv(nv).gt.nvh) &
              stop 'in EXCITE: CAS RANGE'
        end if

!       **** check and print excitation generators ****

        if (nv.gt.1) write (iw,1300) (vv(i),i=1,nv)
        if (vvs(2).gt.0) then
          write (iw,1310) 'SINGLE',(vvs(i),i=1,nvs)
          !call chkdeg (vvs,nvs,aii) ADD
        end if
        if (vvd(2).gt.0) then
          write (iw,1310) 'DOUBLE',(vvd(i),i=1,nvd)
...
!       **** and hence the nber of permutations possible		****
        ...
        nperma= nfact(nv) / nfact(nalpha) / nfact(nv-nalpha)
        npermb= nfact(nv) / nfact(nbeta)  / nfact(nv-nbeta)
        nend= nint (2.D0**nv)
        isynca= nend
...
!         **** spin adapt and include in config list ****
            call spclas (tm,ci,istsym,jconf, aoc,boc,ao1,bo1, e,&
     &          aocc,bocc,nspn,spintr, beta,cc0,gamma,nbtmo,aii, nfact,&
     &          irefd,krefd,.false.)
...
!	          **** spin adapt and include in config list ****
                  call spclas (tm,ci,istsym,jconf, aos,bos,ao1,bo1, e,&
     &              aocc,bocc,nspn,spintr, beta,cc0,gamma,nbtmo,aii, nfact,&
     &              irefd,krefd,.false.)
...
!                       **** proceed only if no single excitation i->b ***
     ...
     &                        i.lt.vvs(1) .or. i.gt.vvs(2)  .or.&
     &                        b.lt.vvs(3) .or. b.gt.vvs(4) )
                      end if
                      if (exok .and. i.eq.b)&
!		        **** proceed only if no single excitation j->a ***
     &                  exok= (.not.aoc(j) .or. aoc(a)) .and.&
     &                        (.not.boc(j) .or. boc(a) .or.&
     &                        j.lt.vvs(1) .or. j.gt.vvs(2) .or.&
     &                        a.lt.vvs(3) .or. a.gt.vvs(4) )
...
!                       **** spin adapt and include in config list ****
                        call spclas (tm,ci,istsym,jconf, aod,bod,ao1,bo1,&
...
!     **************** reorganize basis set order **********************
      ...
      jconf=  nconf
      write (iw,1320) nconf,(nmrep(k,nptg),nci(k),k=1,nr)
...
!     **** determine if state is in a diagonal fragment block ****
      ...
      do i= 1,nconf
        ...
        icifrag(i)= 0
        do j= 1,nov
          if (aocc(j,1,i) .xor. bocc(j,1,i)) then
...
!     **** this checks an exc list for degenerate boundary MO eigenvalues ****
      ...
      integer            i, j, k, idir, ifn, nv

      data tol  /1.d-1/
 1010 format (/' CI WARNING: MO',i4,' EVAL DIFF <',f7.4)
 1011 format (/' CI WARNING: MO',i4,' EVAL DIFF <',f7.4,&
     &         ': EXCITATION RANGE NARROWED')


      do j= 1,4
        i= vv(j)
        idir= 0
        if ( (j.eq.1 .or. j.eq.3) .and. i.gt.1) then
          diff= abs (evalmo(i)-evalmo(i-1))
          if (diff .lt. tol) then
            idir= 1
            ifn= n
          end if
        else if ( (j.eq.2 .or. j.eq.4) .and. i.lt.n .and. i.ge.1) then
          diff= abs (evalmo(i)-evalmo(i+1))
          if (diff .lt. tol) then
            idir= -1
            ifn= 1
        ...
        if (idir.ne.0) then
          if (diff.gt.tol/5.) then
            write (iw,1010) i,tol
          else
            write (iw,1010) i,tol/5.
            do k= i,ifn,idir
              if (abs(evalmo(k)-evalmo(i)).lt.tol) vv(j)= vv(j) + idir
            end do
          end if
        end if
      end do
...
!     **********************************************************************
      ...
      subroutine spclas (tm,ci,istsym,jconf, ao,bo,ao1,bo1, e,&
...
!     **** MP2 storage in excite and spcls ****
      ...
      double precision   s2(mspn,mspn), eval(mspn), wk(mspn), &
                         ...
                         aii(n), spintr(mspn,nov), ci(nconf*(nconf+1)/2), &
                         tm(nb2,3), ecoupl(nconf)
      integer            i, j, k, i0, i1, idet, ier, iisym, ih, il, ip, &
                         irefd, isync, j1, jconf, jdet, &
                         k1, k2, mmp2, nalpha, nbeta, ndeg, &
                         nelj, neltot, nend, nperm, npermo, nsng
...
!     *********** check that its of the correct symmetry ************
      iisym= 1
      do j= 1,nov
        if (ao(j) .xor. bo(j)) iisym= ixprd (iisym,nsym(j+ncore))
      end do
...
!     **** determine nber of sngl occ alpha and beta orbitals ****
      ...
      do i= 1,nov
        if (nel(i).eq.1) then
          nsng= nsng + 1
          if (nsng.gt.mspn) stop 'NSNG in SPCLAS'
          isng(nsng)= i + ncore
          if (ao(i)) then
            nalpha= nalpha + 1
          else
...
!	  **** first, ensure that only 2 alpha orbs are different ****
          ndiff= 0
          do k= 1,nov
            if (ao1(k,i) .xor. ao1(k,j)) ndiff= ndiff + 1
          end do
          s2(i,j)= 0.D0
...
!     **** if degenerate energy eigenvalues, then form S**2 in eigvec basis ****
      ...
      tol= 1.D-5
      il= 1
      do while (il.lt.nperm)
        ih= il + 1
        do while (ih.le.nperm .and. abs(eval(il)-eval(ih)) .lt. tol)
          ih= ih + 1
        end do
        if (ih.gt.nperm .or. abs(eval(il)-eval(ih)) .gt. tol) ih= ih - 1
        ndeg= ih - il + 1
        if (ndeg.gt.1) then
...
!     **** check for non-integral multiplicity ****
      ...
      do i= 1,nperm
        if (abs(mod(wk(i)+tol,1.D0)) .gt. 2*tol) then
...
!	    *********** store the state in CI master index ***********
            ...
            call sortst (tm,jconf, eval(ip),e, ao1,bo1,aocc,bocc, nperm,&
...
!     **** ISYNC is increased one per call; IMAX is terminating value **
      ...
      implicit none
      integer           isng(nov), i, j, k, &
                        ialpha, ibeta, iisym, imax, io, isync, &
...
100   continue
      ...
      do i= 1,ntot
        ...
        if (ao(io)) then
          ialpha= ialpha + 1
          if (ialpha.gt.nalpha) goto 100
        else
          ibeta= ibeta + 1
          if (ibeta.gt.nbeta) goto 100
        end if
      end do
...
!     **********************************************************************
      ...
      function permca (aoi,ao,nalpha,icas,ntot,isync)
...
!     **** ISYNC is decreased one per call; 0 is terminating value	***
      ...
      logical*1         permca,aoi(nov),ao(nov)
      dimension         icas(nov)
...
!     **********************************************************************
      ...
      subroutine sortst (tm,k, ener,e, ao1,bo1,aocc,bocc, nperm,nspn,&
...
!     **** a new state is inserted into the list in energy order	****
      use reimers_C, only: n, nconf, nci, icisym, emaxci, nov, fastci, &
          nb2, nex, mspn
     ...
     do j= k,i,-1
       ...
       do ispn= 1,nspn(j)
         ...
         do jo= 1,nov
           ...
           bocc(jo,ispn,j1)= bocc(jo,ispn,j)
       !ADDe(j1)= e(j)
       !ADDistsym(j1)= istsym(j)
       !ADDnspn(j1)= nspn(j)
       !ADDkrefd(j1)= krefd(j)
       !ADDdo ispn= 1,nspn(j)
       !ADD  spintr(ispn,j1)= spintr(ispn,j)
       !ADD  do jo= 1,nov
       !ADD    aocc(jo,ispn,j1)= aocc(jo,ispn,j)
       !ADD    bocc(jo,ispn,j1)= bocc(jo,ispn,j)
         end do
       end do
     end do
