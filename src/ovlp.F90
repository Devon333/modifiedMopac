...
!     phase (-1)**(l-m) on la.ge.the other l value
!
      if (la.eq.jb) jm= jlp1
      if (la.eq.kb) jm= klp1
...
!.......................................................
      rij= abs (rij)
      if (atomk.eq.atomkx.and.atomj.eq.atomjx) go to 46
      atomkx= atomk
      atomjx= atomj
      ss= zero
      sp= zero
...
!
      issmax= 1
      if (nbt(i).gt.3) issmax= 2
      jssmax= 1
      if (nbt(j).gt.3) jssmax= 2
...
! RMG - Ag overlap scaling
      if (agovlp) then
        if ((knat.eq.47.or.jnat.eq.47).and.knat.ne.jnat) then
          s(nm) = s(nm)*agfact
        end if
      end if
...
!      implicit          double precision (a-h,o-z)
      ...
      data zero,one,two /0.0D0,1.0D0,2.D0/
        ...
        rho2= pp*tt
        ir= dabs(rho2+rho2)
        if (ir.gt.170) go to 100
        is= min0(ir+1,15)
!
!     first the more difficult b-fn.
!
        if (dabs(rho2).lt.0.001) go to 35
           if (pp.gt.40.0D0) go to 100
25            d= dexp(rho2)
              h= dexp(-rho2)
              r= d-h
!
...
!      implicit          double precision (a-h,o-z)
     ...
     &  (f3/f5)*(f4/f6)*rhoa*rhob)/2.D0**(l1+l2+1)
      ...
      if (mod(ju+ku,2).eq.1) strad1= - strad1
...
!      implicit          double precision (a-h,o-z)
   ...
   25 count=0.0D0
      ...
      if (mod(i2+i5+i6+i4+ie+id,2).eq.1) term= - term
