...
!
      ...
      if (id == 1) then
        write (iw, '(/1X,       ''NUMBER CELL  ATOM '',5X,''X'',12X,''Y'',12X,''Z           Tot.'',/)') 
      else
      ...
      if (l123 == 1) then 
        ...
        if (id == 1) then
          ...
          elemnt(nat((i-1)/l123+1)), (dxyz(j,i),j=1,3), sqrt(dxyz(1,i)**2 + dxyz(2,i)**2 + dxyz(3,i)**2),i=1,numtot)
        else
          write (iw, '(I6,4x,a2,F13.6,2F13.6)') (i,elemnt(nat((i-1)/l123+1)), &
          (dxyz(j,i),j=1,3),i=1,numtot) 
        end if
      else 
        write (iw, '(I6,4x,a2,F13.6,2F13.6)') (i,elemnt(nat((i))),(dxyz(j,(i - 1)*l123 + icuc),j=1,3), i=1,numat) 
      endif 
      if (id == 0) return  
      write (iw, &
