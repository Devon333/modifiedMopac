...
!         **** beta differs from first ref det ****
      ...
      if (nio.ne.niv) write (6,*) 'Error: Different number of occ ',&
      ...
      use reimers_C, only: n, nconf, ncore, nov, nex, matind, nvl, mspn,&
                           aor1, bor1
      ...
      integer      :: iconf, io(4), iv(4), nm, nio, niv
      real(double) :: ener
!***********************************************************************
...
! Compute the energy change
      ...
      use reimers_C, only: nconf, matind, nov, nex, mspn, aor1, bor1
      USE chanel_C, only : iw
      implicit none
...
! with the new CI matrix to get uncorrected energies
      ...
      USE vast_kind_param, ONLY:  double
      use molkst_c, only : mpack
      use reimers_C, only: n, nconf, nvl, ncore, nov, nex, &
          matind, mspn, aor1, bor1
      implicit none
...
! coefficient
      ...
      implicit none 
      real(double)  :: deltap(mpack),a(mpack),dp2(n,n),dp3(mpack)
      integer :: i, j, ia, occ, vir, &
                 io(4), iv(4), nio
      real(double) :: nsqa, ffact, dcie
!***********************************************************************
...
!***********************************************************************
      ...
      USE vast_kind_param, ONLY:  double
      use cosmo_C, only : amat, bmat, ffact, nden, gden, ipiden, nps, &
           nsetf, fnsq
      use common_arrays_C, only : nfirst, nlast
      use molkst_c, only : norbs, numat, mpack, keywrd
      use funcon_C, only: a0, ev
      use reimers_C, only: n, matind
      implicit none
...
!***********************************************************************
      ...
      use molkst_c, only : mpack
      use funcon_C, only: a0, ev
      use reimers_C, only: n, nov, cc0, ncore, matind

      implicit none
      real(double)  :: p(mpack), qsc(nps), phi(nps), qden(nden)
      logical*1     :: ao(nov),bo(nov)
      integer :: a, i, j, k, m, ia, k0, kocc, sum
