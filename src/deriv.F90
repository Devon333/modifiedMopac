...
!-----------------------------------------------
      integer :: idelta, icalcn, i, j, nstep, nw2, ncol, icuc
      ...
      logical :: scf1, halfe, slow, aifrst, debug, precis, intn, geochk, ci, &
        aic, noanci, field, saddle, DH_correction, large, l_redo_bonds
...
!
  ...
  !
      ...
      if ((DH_correction .or. Abs (pressure) > 1.d-4) .and. debug) then  
        ...
        write (iw, '(/3X,       ''NUMBER    ATOM '',5X,''X'',12X,''Y'',12X,''Z           Tot.'',/)') 
        if (l123 == 1) then 
          write (iw, '(I6,4x,a2,F13.6,2F13.6)') (i,elemnt(nat(i)),(dxyz(j + (i - 1)*3), j = 1,3), i = 1, numat) 
        else if (large) then 
          icuc = (l123 + 1)/2 
          if (id == 1) then
            write (iw, '(I6, i4, 4x,a2,F13.6,3F13.6)') (((i-1)/l123+1), mod(i - 1, l123) - icuc + 1, &
            elemnt(nat((i-1)/l123+1)), (dxyz(j + (i - 1)*3), j = 1, 3), &
            sqrt(dxyz(1 + (i - 1)*3)**2 + dxyz(2 + (i - 1)*3)**2 + dxyz(3 + (i - 1)*3)**2), i = 1, numat*l123)
          else
