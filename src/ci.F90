...
!     **** ie, <ia||jb>= (ia||jb), and parameter definition reverses ****
      use reimers_C, only: n, matind, nham, ibf, iat, nslwr, nsupr, nstr, &
...
!	**** see if element already evaluated and stored ****
        ...
        edefs= edef / 2.
        if (ee2(nme).lt.edefs .and. ee2(nmc).lt.edefs) then
!	  **** retriev from store ****
          j12= ee2(nmc)
          k12= ee2(nme)
          return
...
!     ********** evaluate integrals **************
      ...
      diagon= i.eq.j .and. a.eq.b
      j12= 0.D0
      k12= 0.D0

!     **** elements of symmetry i x j ****

      if ( iand (nbtmo(i),nbtmo(j)) .ne. 0) then
...
!     **** integrals of symmetry i x a ****
      ...
      if ( iand (nbtmo(i),nbtmo(a)) .ne. 0) then
...
!     **** integrals (mu,nu|nu,mu) from INDO, symmetry= i x b ****
      ...
      if (.not.diagon .and. nham.eq.2 .and.&
     &                iand (nbtmo(i),nbtmo(b)) .ne. 0) then
        jk12= 0.D0
        imu0= 0
...
!	**** electron is beta spin, move past an alpha spin in orb j1 ****
      ...
      align= 1.D0
      if (mod(ninter,2).eq.1) align= -1.D0
      return
...
!     **********************************************************************
     ...
     &                spintr,nspn, ci, ndump)
      ...
      double precision  e(nconf), beta(nb2), ci(nconf*(nconf+1)/2), &
                        ...
                        b, ciener, e1, e2, xxx
      integer           istsym(nconf), nspn(nconf), io(nconf), &
                        iv(nconf), i, j, k, iii, iout, ioutt, iw, &
        ...
        do k2= 1,nconf
          ...
          do k1= 1,k2-1
            ...
            if (istsym(k2).eq.istsym(k1)) then
              ...
              iii= 0
              if (abs(e(k1)*e(k2)).lt.1.d-3) iii= 1
...
!	      **** are both eigenvectors of the same orb class in SPCLAS ***
              same= nspn(k1) .eq. nspn(k2)
              i= 0
              do while (same .and. i.lt.nov)
                i= i + 1
...
!       **** dump matrix elements differing by 1 electron ****
        ...
        do i= nol,nvh
          do a= nol,i
            ...
            if (iout.eq.5 .or. iout.gt.0 .and. nm.eq.nxs) then
              write (iw,1020) (iwk(1,k),iwk(2,k), wk0(1,k),wk0(2,k),&
     &            wk0(1,k)+wk0(2,k),k=1,iout)
              iout= 0
            end if
          end do
        end do
        write (iw,1030) 'eec',ioutt,nxs,ioutt/float(nxs)
        ...
        do i= nol,nvh
         do j= nol,i
          do a= nol,j
           do b= nol,a
            nm= nm + 1
            if (ee2(nm).lt.edefs) then
              ioutt= ioutt + 1
              if (ee2(nm).ne.0.E0) then
                iout= iout + 1
                iwk(1,iout)= i 
                iwk(2,iout)= j 
                iwk(3,iout)= a 
            ...
            if (iout.eq.6 .or. iout.gt.0 .and. nm.eq.nxss) then
              write (iw,1021) (iwk(1,k),iwk(2,k),iwk(3,k),iwk(4,k),&
     &            wk0(1,k),k=1,iout)
              iout= 0
            end if
           end do
        ...
        write (iw,1030) 'ee2',ioutt,nxss,ioutt/float(nxss)
      end if

 1010 format (/' Contributions to integrals differing by 1 electron:',&
     &         ' from BETA, from sum over CI core, and total'/)
 1011 format (/' Non-Zero Contributions to integrals differing by 2',&
     &         ' electrons'/)
 1020 format (5(2i4,2f6.0,f6.1))
 1021 format (6(4i4,f6.2))
 1030 format (1x,a3,' used=',i6,' maximum=',i6,' fraction=',f6.2)
      return
      end
...
!      USE chanel_C, only : iw
      ...
      double precision  beta(nb2), c(n,n), gamma(n,n), &
                        j12, k12, ciener, e, eec1, a, align
      integer           i, j, ivo, j1, ndiffo, ndiffv
      logical*1         aoi(nov), boi(nov), aoj(nov), boj(nov),&
     &                  ao, bo, spno(2), spnv(2), ao1(nov), bo1(nov)
      real(double)      ediel, fcon, phi(nps), qden(nden), p(mpack)
...
!        **** check alpha occupancy ****
        if (aoi(j) .xor. aoj(j)) then
          if (ndiff.eq.4) goto 120
          ndiff= ndiff + 1
          if (aoi(j)) then
            ndiffo= ndiffo + 1
            if (ndiffo.gt.2) goto 120
            io(ndiffo)= j1
            spno(ndiffo)= .true.
          else
            ndiffv= ndiffv + 1
            if (ndiffv.gt.2) goto 120
            iv(ndiffv)= j1
            spnv(ndiffv)= .true.
          end if
        end if
...
!	**** (io,iv|j,j) terms with j in xcitn space ****
        do j1= nol,nvh
          ...
          ao= aoi(j)
          bo= boi(j)
          if (j1.eq.io(1) .or. j1.eq.iv(1)) then
...
!     *************************************************************************
      ...
      use reimers_C, only: n, matind, nconf, nci, nr, nov, nex, mspn 
!      USE chanel_C, only : iw
      use cosmo_C, only: ffact, diagsl
      ...
      integer           i, j, k, i0, i1, ier, nm, nconf1
...
!     *************************************************************************
      ...
      subroutine ciout (evalmo,ci,evalci,istsym,dmci,aocc,bocc,tot)
      use reimers_C, only: n, matind, au2ev, au2ang, au2cm, debye, &
          nconf, multci, ndtype, nmrep, nptg, nr, ixprd, ncore, nov, &
          n2phot, nese, nciout, nciouv, filenm, lenf, irrtyp, occ, &
          istate, nex, mspn, nsym
      USE chanel_C, only : iw
      ...
      double precision  mom, evalci(nconf), ci(nconf,nconf), &
                        dmci(nconf*(nconf+1)/2,3), evalmo(n), &
                        beta, deldip, dipch, dlo, dmtot, eng, feps, &
                        fosc, foscf, foscl, foscm, fref, freq, freqk, &
                        rmom, solva, solr, soleps, solref, solve, tot, &
                        wl, cisum
 ...
 1000 format (/'After CI for multiplicity=',i2,' EXCITING FROM state',&
     &        i3,' at Energy=',f10.0,' of symmetry ',a3)
 1002 format (/'  CI trans.  energy frequency wavelength oscillator',&
     & ' ---------polarization---------    dipole  ',&
     & ' ------components----- ')
 1102 format (' st.  symm.    eV      cm-1       nm      strength ',&
     & '      x          y         z       moment     x       y  ',&
     & '     z ' /)
 1003 format (i4,1x,a3,f11.7,f10.0,f10.2,f10.6,3f11.6,f10.6,3f8.3)
 1004 format (i4,1x,a3,f11.7,f10.0,f10.2,f10.6,33x,   f10.6,3f8.3)

 1005 format (a5,1x,i4,1x,f7.4,'     CI coeff  CI percent')
 1006 format (4x,a7,2x,i5,1x,f12.8,f12.8)
 1007 format (a24,7x,f12.8)
 1011 format (/' CI coefficient matrix of symmetry ',&
     &  'species ',a3//'   determ-'/'   inate', 6x,51(1h-),' CI state',&
     &  54(1h-) )
 1012 format (/6x,12i10/)
 1013 format (5x,i4,12f10.5)
 1017 format (1h1,10x,a80,5x,2(i2,'/'),i2/)
 1040 format (/' Using Rettig non-polarizable spherical cavity ',&
     &    'solvent shift formula with' /&
     &    ' r=',f6.3,' static diel const=',f6.1,' refr index=',&
     &    f6.3,' f(eps)=',f10.0,' f(n**2)=',f10.0)
 1100 format (/'1 post-core MO orbital occupations in CI states of'&
     &        ' multiplicity=',i4 // ' MO=',10x,19i6 :/ (14x,19i6) )
 1110 format (' MO Symm=',7x,19(2x,a3,1x) :/ (17x,19(2x,a3,1x)) )
 1120 format (' MO energy=',4x,19f6.1 :/ (15x,19f6.1) )
 1130 format (i4,1x,a3,f7.0,19f6.2 :/ (15x,19f6.2) )
 1150 format (/' Depression of ground-state after CI=',&
     &         f12.0,' cm**-1',' Energy=',f14.7,' eV' /&
     &         ' Dipole moment=',3f10.6,' tot=',f10.6,' Debyes'/)
 1151 format (' Ground state is ',a3,' Depression=',f8.0)
 1170 format (i4,i2,1x,a3,f8.1,f10.6,f8.3)
 2000 format (f8.0,' -4' / f8.0,f7.3 / 'tu 21 ',f8.0,f7.3,' 0 0.2 "'&
     &    ,a4,'"' / 'dr')
 2010 format ('co states with Fosc>0.00001' /&
     &  'xr 0 50' / 'yr -4 1' / 'xl \gn\r / 1000 cm\u-1' /&
     &        'yl log (f\dosc\c)' /&
     &  '  0 -4' / '  50 -4' / 'dr' )
...
!     **** read solvent shift parameters ****
      ...
      solv = .false.
      if (solv) then
!	**** solvent shift Rettig J.Mol.Str. 84 (1982) 303 ****
        read (ix,*) solr,soleps,solref
        feps= au2cm * au2ang * 2.D0 / solr**3
        fref= feps * (solref**2-1.0) / (2.D0*solref**2+1.D0)
        feps= feps * (soleps-1.0)    / (2.D0*soleps+1.D0)
        write (iw,1040) solr,soleps,solref,feps,fref
      else
        ssolv= ' '
        solva= 0.D0
        solve= 0.D0
...
! End RMG
          ...
          wl= 0.D0
          if (abs(freq).gt.1.0) wl= 1.D7/abs(freq)
          nsy= istate(il)
          nst= ixprd(nsx,nsy)
...
!	    **** f= 2/3*m**2*e with m, e in au; with m in Ang and e in cm**-1
            fosc= foscf * freq * mom
            if (ndtype.eq.3) fosc= fosc * 1000.D0
          else
!	    **** f= 2/3*m**2/e with m, e in au; with m in Ang**-1 and e in cm**-1
     ...
     &      (dmci(nmd,2)-dmci(1,2))**2 + (dmci(nmd,3)-dmci(1,3))**2)
          if (irrtyp.gt.0 .and. freq.lt.4.D5) write (40,1170)&
     &      il-1,nst,nmrep(nst,nptg),freq,fosc,dipch

          if (il.gt.i0 .and. fosc.gt.0.00000001) then

!	    **** output to plot file ****
            foscl= log10 (fosc)
            freqk= freq / 1000.
            if (  abs(dmci(nm,1)) .gt. abs(dmci(nm,2))) then
              if (abs(dmci(nm,1)) .gt. abs(dmci(nm,3))) then
                pdir= 'x'
              else
                pdir= 'z'
              end if
            else if (abs(dmci(nm,2)) .gt. abs(dmci(nm,3))) then
              pdir= 'y'
            else
              pdir= 'z'
            end if
            if (nmrep(nst,nptg).eq.'   ') then
              plabl= pdir
            else
              plabl= pdir // '-' // nmrep(nst,nptg)
            end if

!	    **** main output to log file ****
            rmom= 1.D0 / sqrt(mom)
            if (ndtype.le.1) then
              write (iw,1003) il,nmrep(nst,nptg),eng,freq,wl,fosc,&
     ...
     &                (dmci(nm,kk)*rmom,kk=1,3)
          ...
          else if (il.gt.i0) then
!	    **** main output to log file, no osc strength ****
            if (ndtype.le.1) then
              write (iw,1004) il,nmrep(nst,nptg),eng,freq,wl,fosc,&
     &                dmtot,(debye*dmci(nmd,kk),kk=1,3)
            else
              write (iw,1004) il,nmrep(nst,nptg),eng,freq,wl,fosc
            end if
        ...
        if (ndtype.le.1) call polout
        if (n2phot.gt.0) call twopho (i0,evalci,dmci,istate)
      end do
...
!
        cisum = 0
        do i= 1,nconf
          if (abs(ci(il,i)).gt.wrtconf) then
...
!     **** between CI configurations					****
      use reimers_C, only: n, na, nb2, matind, au2ev, au2ang, au2cm, debye, &
 ...
 800  format (' The lowest ',i4,' spin-adapted configurations of ',&
     &        'multiplicity=',i3 /&
     &        5x,'sym   eV   cm**-1 -dets- dipole oscilator X FRAG ....',&
     &        'Excitations named from first reference determinate' /&
     &        23x,'tot  #  Debye  strength') 
 801  format  (45x,' x 1000')
 811  format (i4,1x,a3,f7.3,f8.0,2i3,f8.4,f9.6,i2,i3,&
     &        ' (',20a4)
 1002 format (1h1,10x,a80,5x,2(i2,'/'),i2/)

      write (iw,800) nconf,multci
      nciouv = nconf

      if (ndtype.eq.3) write (iw,801)
      write (iw,*)
      call polzro
      sqrt2= sqrt (2.D0)
      foscf= 2.D0/3.D0/au2ang**2/au2cm
...
!	      **** count the nber of electrons differing, proceed if 1 only ****
              ndiff= 0
              k= 0
              do while (ndiff.le.3 .and. k.lt.nov)
                k= k + 1
                adiff= aocc(k,i1,i) .xor. aocc(k,j1,j)
                bdiff= bocc(k,i1,i) .xor. bocc(k,j1,j)
...
!     **** first, calc max nber of excitations for format spec ****
     ...
     &                      (bocc(j,1,i),j=1,nov)
      ...
      if (maxx.gt.9) stop 'EXCITE- OUTPUT IS FOR MAX OF 9 EXCITNS !'
...
!	**** check if det list is same as any previous config ****
        nspni= 1
        do k= 1,i-1
          same= nspn(k) .eq. nspn(i)
          j= 0
          do while (j.lt.nov .and. same)
            j= j + 1
...
!	  **** f= 2/3*m**2/e with m, e in au; with m in Ang**-1 and e in cm**-1
        ...
        write (iw,811) i,rep,e(i),freq,nspn(i),nspni,diplen*debye,&
...
!     **** two-photon intensities ****
      if (n2phot.gt.0) call twopho (ilow,e,dmci,istsym)
...
!     **** Electrodynamics (Academic, London, 1984) P109		****
 ...
 1010 format ('1Single-Beam 2-photon relative Einstein B coefficients',&
     &        ' exciting from state',i4 /&
     &        ' transition  energy  lin    cir!       xx        xy',&
     &        '        yy        xz        yz        zz' / )
 1020 format (i4,1x,a3,f8.3,2f8.3,6f10.7)
...
!	  **** double count diag terms, calc single-beam intensities ****
     ...
     &       (alp21**2 + alp31**2 + alp32**2)
          ...
          write (iw,1020) j,nmrep(isym,nptg),ener(j)-ener(i0),planar,&
     &      circ,alp11,alp21,alp22,alp31,alp32,alp33
        end if
      end do
