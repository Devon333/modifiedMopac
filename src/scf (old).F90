...
! RMG - need this for first scf cycle
...
! ____________________________________________________________________________
      ...
      double precision  notype(8,5),notot(8)
      integer           i, j, k, l, m, ii, ir, is, kst, &
                        nhomo, nlumo, nmoou1, nmoou2

! RMG - switch coefficients of C to account for MOPAC
...
!     ***** page eject for MO dump ****
      ...
      if (nhomo.ge.1 .and. nlumo.le.n) then
        write (iw,1180) aii(nhomo),aii(nlumo),aii(nlumo)-aii(nhomo)
      end if
...
!	  **** no hybridization contribution from this atom ****
          write (iw,1175) i,iatsym(nati),pt,qgs(i)
        else
!	  **** determine hybridization contribution ****
          ...
          write (iw,1175) i,iatsym(nati),pt,qgs(i),(dipgs(i,k),k=1,3)
        end if
      ...
      write (iw,'(//)')
      write (iw,1004) dc,cm, dp,pm, dt,dip0

      return
      end

! RMG subroutine - compute and write bond orders
      subroutine boa (s,c)
