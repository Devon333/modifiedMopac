        ...
        do i= nslwr(ir),nsupr(ir)
          do j= i,nsupr(ir)
            ...
            do i1= 1,nstr(i)
              ...
              do j1= 1,nstr(j)
                jb= abs(istr(j1,j))
                if (jb.gt.ib .and. ihalf.eq.2 .or.&
     &              jb.le.ib .and. ihalf.ne.2) then
...
!     **** transform integrals (i,j|i,j) in upper half ****
      if (nham.eq.2) call st (g1,gamma,2)
...
!     *************************************************************************
      ...
      integer           i, j, k, i0, i1, i2, ibtype, ip, ir, &
                        k1, k2, nm, nn, nnb2
      character*(*)     name
      character*21      f1150
 ...
 1151 format (1x, i6,1x, a2,2x,15f8.4)
 1152 format (1x, i6,       5x,15f8.4)
 1160 format (' Block of symmetry ',a4)
...
!     **** find largest elements ****
      ...
      do while (k2.lt.nn)
        ...
        k2= min (k1+14,nn)
        write (iw,*)
        write (iw,1145) (i,i=k1,k2)
        if (ibtype.lt.3) then
          do j= k1,nn
            nm= matind(j)
            if (ibtype.eq.0) then
!	      **** generic matrix ****
              write (iw,1151) j,'  ',(a(nm+k),k=k1,min(j,k2))
            else if (ibtype.eq.1) then
!	      **** atom by atom matrix ****
              write (iw,1151) j,iatsym(natm(j)),(a(nm+k),k=k1,min(j,k2))
            else
!	      **** orbital by orbital matrix ****
     ...
     &                                (a(nm+k),k=k1,min(j,k2))
              if (nham.ne.1 .and. nham.ne.3 .and. (j.eq.nn .or.&
     &           i.ne.iat(j+1)) ) write (iw,*)
            end if
          end do
...
!	  **** configuration by configuration matrix, symmetry-block wize ****
          ...
          do ir= 1,nr
            if (nci(ir).gt.0) then
              if (nptg.gt.1) write (iw,1160) nmrep(ir,nptg)
              i1= i0 - 1
              i2= i1 + nci(ir)
              if (ibtype.eq.4) i2= min (i2, i1+14)
              do i= i1+1,i1+nci(ir)
                nm= matind(i) + i1
                write (iw,1152) i,(a(nm+k),k=1,min(15,i-i1))
              end do
              i0= i0 + nci(ir)
            end if
          end do
