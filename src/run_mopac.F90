...
!
      USE molkst_C, only : gnorm, natoms, numat, nvar, numcal, job_no, nscf, id, &
        ...
        density, norbs, method_indo, nclose, nopen, norbs
!
...
!
      ...
      logical :: exists, opend, sparkles_available
...
! Set up CI data
        if (index(keywrd,' CIS') /= 0 .or. index(keywrd,' MRCI') /= 0 .or. &
          ...
          if(.not. allocated(evalmo)) allocate(evalmo(norbs))
...
!
        if (index(keywrd, " OLDDEN") + index(keywrd, " NEWDEN") == 0) then
          i = len_trim(end_fn)
          open(unit=iend, file=end_fn(:i - 3)//"den", status='OLD', iostat=i)
